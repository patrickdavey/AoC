defmodule AOC.Board do
  ...
  defp get_row(y, board, min_x, max_x) do
    ...
    |> Enum.map(&fetch_piece/1)
    |> Enum.join("")
  end

  defp fetch_piece({:wall, _}), do: "#"
  defp fetch_piece({:empty, _}), do: "."
  defp fetch_piece({:us, _}), do: "@"
  defp fetch_piece({:door, door}), do: door
  defp fetch_piece({:key, key}), do: key
end
